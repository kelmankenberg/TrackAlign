...
- `AppPreferences`: persisted user settings including a theme preset (`light-paper`, `light-warm`, `light-blue`, `light-high-contrast`, `dark-moss`, `dark-vscode`, `dark-charcoal`, or `dark-midnight`), default rename template, navigation layout, and other supported preferences.
