...
function Workspace({ onHelp, onStatusChange }: { onHelp: () => void; onStatusChange: (status: string) => void }) {
  ...
  const proposals: MatchProposal[] = automaticProposals.map((proposal) => {
    if (excludedPaths.has(proposal.filePath)) return { ...proposal, trackPosition: null, trackTitle: '', trackArtist: '', status: 'excluded' as const, score: 0, evidence: [] }
  ...
  return <section className="workspace-page">
    ...
    {inventory && collection && <div className="review-preview"><div className="preview-heading"><div><span className="eyebrow">Alignment review</span><h2>Proposed matches</h2></div><span className="status-pill"><span className="status-dot" />{proposals.filter((proposal) => proposal.status === 'matched' || proposal.status === 'manual').length} assigned</span></div><div className="review-table"><div className="review-header"><span>Local file</span><span>Spotify match</span><span>Confidence</span><span>Status</span></div>{proposals.map((proposal) => <div className="review-row" key={proposal.filePath}><span className="review-file">{proposal.fileName}</span><select className="match-select" value={proposal.status === 'excluded' ? 'excluded' : proposal.trackPosition ?? ''} onChange={(event) => { if (event.target.value === 'excluded') setExcludedPaths((current) => new Set(current).add(proposal.filePath)); else { setExcludedPaths((current) => { const next = new Set(current); next.delete(proposal.filePath); return next }); setManualMatches((current) => ({ ...current, [proposal.filePath]: event.target.value ? Number(event.target.value) : null })) } }} aria-label={`Match for ${proposal.fileName}`}><option value="">No match</option><option value="excluded">Exclude file</option>{collection.tracks.map((track) => <option key={track.position} value={track.position}>{String(track.position).padStart(2, '0')} · {track.artist} — {track.title}</option>)}</select><span>{Math.round(proposal.score * 100)}% {proposal.evidence.length ? `· ${proposal.evidence.join(', ')}` : ''}</span><span className={`match-status ${proposal.status}`}>{proposal.status}</span></div>)}</div></div>}
