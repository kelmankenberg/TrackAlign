...
function Workspace({ onHelp, onStatusChange, renameTemplate }: { onHelp: () => void; onStatusChange: (status: string) => void; renameTemplate: string }) {
  ...
  const [searchHistory, setSearchHistory] = useState<string[]>(() => {
    try {
      const parsed = JSON.parse(localStorage.getItem('trackalign-search-history') || '[]')
  ...
  const recordSearchHistory = (query: string) => {
    setSearchHistory((current) => {
      const next = [query, ...current.filter((entry) => entry.toLowerCase() !== query.toLowerCase())].slice(0, 10)
      localStorage.setItem('trackalign-search-history', JSON.stringify(next))
      return next
    })
  }
  ...
  const runSpotifySearch = async () => {
    ...
    try {
      ...
      const results = await window.trackAlign.spotify.search(trimmedQuery, types)
